...
from ct_reconstruction.xcat_geom import XCATGeometry
from ct_reconstruction.ring_corr import ring_correction
import general_functions as gen
from ct_reconstruction.convert_air_dark import convert
from ct_reconstruction.find_dead_pixels import find_dead_pixels, find_dead_pixels_all_bins
...
def prepare_sinogram(airtime=60, duration=scan_duration, num_proj=num_angles, n_bins=num_bins, corr_rings=True,
    ...
    if corr_rings:
        ...
        if os.path.exists(os.path.join(directory, folder, 'corr_array.npy')):
            ...
            corr_array = ring_correction(corr_data, air_data * (airtime / proj_time), dpm, num_bins=n_bins)
            if save:
